%% Step 4: IQ----Tower Rearranging: 21004/6383
clear
clc

%% load data
load /data/users2/zfu/Matlab/GSU/UKB/Cannabis/Codes/Step3_demo_FNC.mat
...
for s_sub = 1:size(ICA_fold_out_use,1)
    ...
    if strcmp(tmp_sess, 'ses_01')
        ...
        input_IQ(s_sub,1) = table2array(demo_ana1(s_sub,2508))./table2array(demo_ana1(s_sub,1591));

    elseif strcmp(tmp_sess, 'ses_02')
        input_age(s_sub,1) = table2array(demo_ana3(s_sub,5));
        input_IQ(s_sub,1) = table2array(demo_ana1(s_sub,2509))./table2array(demo_ana1(s_sub,1592));
...
for i = 1:1378
        ...
        tbl = table(input_feature(use_idx), input_IQ(use_idx), input_age(use_idx), input_gender(use_idx), input_ID(use_idx), input_meanFD(use_idx), 'VariableNames',{'Feature', 'Score', 'Age', 'Gender', 'ID', 'MFD'});
        lme = fitlme(tbl,'Feature~Score+Age+Gender+MFD');
...
FNC_disp = (vec2mat(asso_FNC.pval,53) <= 1).*vec2mat(asso_FNC.rval,53);
Draw_FNC_Trends(FNC_disp, domain_Name, domain_ICN, [-0.05 0.05])

save('Step4_FNC_vs_IQ1_QC4.mat', 'asso_FNC', 'use_idx')